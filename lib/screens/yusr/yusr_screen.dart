...
class YusrScreen extends StatefulWidget {
  const YusrScreen({super.key});

  @override
  State<YusrScreen> createState() => _YusrScreenState();
}

class _YusrScreenState extends State<YusrScreen> {
        ...
        body: SafeArea(
          child: Column(
            children: [
                    ...
                    return Column(
                      children: [
                        ...
                        if (isFirst && provider.chatMessages.length == 1)
                          _PromptChips(isArabic: isArabic, onChipTap: (text) {
                            _ctrl.text = text;
                            _send(isArabic);
                          }),
                      ],
              ...
              // Crisis card
              if (_showCrisis)
                _CrisisCard(onDismiss: () => setState(() => _showCrisis = false)),
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    GestureDetector(
                      onTap: () => _send(isArabic),
                      child: Container(
                        width: 44,
                        height: 44,
                        decoration: const BoxDecoration(color: AppColors.primary, shape: BoxShape.circle),
                        child: Icon(isArabic ? Icons.arrow_back_ios_new : Icons.send_rounded, color: Colors.white, size: 18),
                      ),
                    ),
                  ],
                ),
...
class _PromptChips extends StatelessWidget {
  ...
  final bool isArabic;
  const _PromptChips({required this.onChipTap, required this.isArabic});

  @override
  Widget build(BuildContext context) {
    final l = AppLocalizations(isArabic: isArabic);
    final prompts = l.yusrPromptChips;
        ...
        children: prompts.map((p) {
          return GestureDetector(
            ...
            child: Container(
              ...
              child: Text(p, style: GoogleFonts.inter(fontSize: 14, color: AppColors.textSecondary)),
            ),
          );
        }).toList(),
      ),
...
class _CrisisCard extends StatelessWidget {
  ...
  const _CrisisCard({required this.onDismiss});
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border(
          right: isArabic ? const BorderSide(color: AppColors.emergencyRed, width: 4) : BorderSide.none,
          left: !isArabic ? const BorderSide(color: AppColors.emergencyRed, width: 4) : BorderSide.none,
        ),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          ...
          Text(
            ...
            style: GoogleFonts.inter(
                fontSize: 18, fontWeight: FontWeight.bold, color: AppColors.emergencyRed),
          ),
